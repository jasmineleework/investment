...
PROMPT='执行 /morning --push：严格按 investment-plugin/skills/morning-update/SKILL.md 的 8 步流程生成晨报并推送 Telegram。must_push 桶（Citrini）文章零丢弃；Discovery 必须附来源文章并先用 discovery_log.py 排重；渲染前必跑 check_urls.py。'
